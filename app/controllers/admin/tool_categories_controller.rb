class Admin::ToolCategoriesController < Admin::BaseController
  ...
  before_action :find_tool_category, only: [ :edit, :update, :position, :destroy ]
  ...
  def tool_category_params
    params.require(:tool_category).permit(:title, :description, :enabled, seo_attributes: [:slug, :title, :keywords, :description, :id])
  end
